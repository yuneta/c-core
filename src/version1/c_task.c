...
SDATA (ASN_OCTET_STR,   "info",                 0,  "", "Info to put in output event"),
SDATA (ASN_JSON,        "jn_info",              0,  0, "Json Info to put in output event"),
SDATA (ASN_UNSIGNED,    "max_count",            0,  0, "Count to reach."),
SDATA (ASN_INTEGER,     "expiration_timeout",   0,  0, "Timeout to finish the task"),

SDATA (ASN_JSON,        "input_schema",         0,  0, "json input schema. See task.h"),

SDATA (ASN_OCTET_STR,   "final_event_name",     0,  "EV_FINAL_COUNT", "final output event"),
...
PRIVATE int mt_stop(hgobj gobj)
{
    ...
    gobj_delete_publisher_subscriptions(gobj, FALSE); // avoid publishing output event.
...
PRIVATE int ac_count(hgobj gobj, const char *event, json_t *kw, hgobj src)
{
    ...
    BOOL trace = gobj_trace_level(gobj);
    ...
    json_array_foreach(priv->jn_input_schema, index, value) {
        ...
        if(strcmp(EvChkItem_event, event2count)==0) {
            if(trace) {
                log_debug_printf("match event", event2count);
            }
            // event matchs
...
PRIVATE GCLASS _gclass = {
    0,  // base
    GCLASS_TASK_NAME,      // CHANGE WITH each gclass
    &fsm,
    {
        mt_create,
        0, //mt_destroy,
