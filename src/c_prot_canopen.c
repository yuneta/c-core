 ...
 *          Prot_canopen GClass.
 *
 *          Prot_canopen uv-mixin for Yuneta
 *
...
//                     "function",     "%s", __FUNCTION__,
//                     "msgset",       "%s", MSGSET_DATABASE_ERROR,
//                     "msg",          "%s", "Prot_canopen connection closed 1",
...
//                                         "function",     "%s", __FUNCTION__,
//                                         "msgset",       "%s", MSGSET_DATABASE_ERROR,
//                                         "msg",          "%s", "Prot_canopen Multiple Results or disconnected",
//                                         NULL
//                                     );
//                                     /*
//                                      *  WARNING
//                                      *  Por aquí crash cuando se desconecta prot_canopen
//                                      */
//                                     set_disconnected(gobj);
//                                     return;
//                                 }
...
//                                 /*
//                                  *  WARNING
//                                  *  Por aquí se queda en bucle cuando se desconecta prot_canopen
//                                  */
//                                 log_error(0,
//                                     "gobj",         "%s", gobj_full_name(gobj),
//                                     "function",     "%s", __FUNCTION__,
...
//             Prot_canopenPollingStatusType st = PQconnectPoll(priv->conn);
//             if(gobj_trace_level(gobj) & TRACE_UV) {
...
//                     "function",     "%s", __FUNCTION__,
//                     "msgset",       "%s", MSGSET_LIBUV_ERROR,
//                     "msg",          "%s", "Prot_canopen connection FAILED",
//                     "error",        "%s", PQerrorMessage(priv->conn),
//                     NULL
//                 );
//                 set_disconnected(gobj);
...
PRIVATE int clear_queue(hgobj gobj, json_t *kw_)
{
    ...
    if(empty_string(id)) {
        if(gobj_trace_level(gobj) & TRACE_MESSAGES) {
            if(priv->cur_query) {
                log_debug_json(0, priv->cur_query, "🗂🗂Prot_canopen CLEAR current query");
            }
            log_debug_json(0, priv->dl_queries, "🗂🗂Prot_canopen CLEAR QUEUE");
        }
        ...
        json_array_foreach(priv->dl_queries, idx, jn_query) {
            ...
            if(id_ && strcmp(id_, id)==0) {
                if(gobj_trace_level(gobj) & TRACE_MESSAGES) {
                    log_debug_json(0, jn_query, "🗂🗂Prot_canopen CLEAR query");
                }
...
)
{
    ...
    if(empty_string(query)) {
        ...
        if(gobj_trace_level(gobj) & TRACE_MESSAGES) {
            ...
            trace_msg("🗂🗂Prot_canopen PUSH QUERY ⏬ dst '%s'\n%s\n", dst?dst:"", query);
        }
    ...
    if(priv->cur_query) {
        if(gobj_trace_level(gobj) & TRACE_MESSAGES) {
            ...
            log_warning(0,
                ...
                "function",     "%s", __FUNCTION__,
                "msgset",       "%s", MSGSET_DATABASE,
                "msg",          "%s", "prot_canopen query in progress",
...
PRIVATE int send_cur_query(hgobj gobj)
{
    ...
    if(gobj_trace_level(gobj) & TRACE_MESSAGES) {
        ...
        trace_msg("🗂🗂Prot_canopen SEND QUERY ⏩ dst %s\n%s\n", dst?dst:"", query);
    }
...
//             "function",     "%s", __FUNCTION__,
//             "msgset",       "%s", MSGSET_DATABASE_ERROR,
//             "msg",          "%s", "Prot_canopen connection closed 3",
//             "error",        "%s", error,
//             NULL
//         );
...
//             log_debug_json(0, kw_result, "🗂🗂Prot_canopen RESULT ⏪ 🔴 ERROR, dst '%s'", dst?dst:"");
//         }
...
//             "function",     "%s", __FUNCTION__,
//             "msgset",       "%s", MSGSET_INTERNAL_ERROR,
//             "msg",          "%s", "Prot_canopen Binary response NOT SUPPORTED",
//             NULL
//         );
//     }
//
...
//                                     "function",     "%s", __FUNCTION__,
//                                     "msgset",       "%s", MSGSET_INTERNAL_ERROR,
//                                     "msg",          "%s", "Prot_canopen type NOT IMPLEMENTED",
//                                     "oid",          "%d", oid,
//                                     NULL
//                                 );
//                                 break;
...
//             log_debug_json(0, kw_result, "🗂🗂Prot_canopen RESULT ⏪ 🔴 ERROR, dst '%s'", dst?dst:"");
//         } else {
//             log_debug_json(0, kw_result, "🗂🗂Prot_canopen RESULT ⏪ 🔵 OK, dst '%s'", dst?dst:"");
//         }
//     }
//
//     return publish_result(gobj, kw_result);
...
//             "function",     "%s", __FUNCTION__,
//             "msgset",       "%s", MSGSET_PARAMETER_ERROR,
//             "msg",          "%s", "Not prot_canopen URI has been configured!",
//             NULL
//         );
//     }
//     if(priv->conn) {
...
PRIVATE int ac_timeout_data(hgobj gobj, const char *event, json_t *kw, hgobj src)
{
    ...
    log_error(0,
        ...
        "function",     "%s", __FUNCTION__,
        "msgset",       "%s", MSGSET_DATABASE_ERROR,
        "msg",          "%s", "Prot_canopen timeout, reset connection",
        NULL
    );
    const char *dst = kw_get_str(kw_result, "dst", "", 0);
    log_debug_json(0, kw_result, "🗂🗂Prot_canopen RESULT ⏪ ⏳TIMEOUT, dst '%s'", dst?dst:"");
